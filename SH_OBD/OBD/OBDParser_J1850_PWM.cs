    ...
    public class OBDParser_J1850_PWM : OBDParser {
        ...
        public override OBDResponseList Parse(OBDParameter param, string response) {
            ...
            if (ErrorCheck(response)) {
                ...
                groups.Add(group);
                if (lines[0].Length < OBDParser_J1850_PWM.HEADER_LENGTH) {
                    responseList.ErrorDetected = true;
                    return responseList;
                }

                string header = lines[0].Substring(0, OBDParser_J1850_PWM.HEADER_LENGTH);
                for (int i = 1; i < lines.Count; i++) {
                    if (lines[i].Length >= OBDParser_J1850_PWM.HEADER_LENGTH) {
                        if (lines[i].Substring(0, OBDParser_J1850_PWM.HEADER_LENGTH).CompareTo(header) == 0) {
                            group.Add(lines[i]);
                        } else {
                            group = new List<string> { lines[i] };
                            groups.Add(group);
                            header = lines[i].Substring(0, OBDParser_J1850_PWM.HEADER_LENGTH);
                        }
                    } else {
                        responseList.ErrorDetected = true;
                        return responseList;
                ...
                for (int i = 0; i < groups.Count; i++) {
                    ...
                    obd_response.Header = groups[i][0].Substring(0, OBDParser_J1850_PWM.HEADER_LENGTH);
